...
@Component({
  selector: 'app-mmht',
  templateUrl: './mmht.component.html',
  styleUrls: ['./mmht.component.css']
})
export class MmhtComponent implements OnInit {

  constructor(private MmhtService: MmhtService, private modalService: BsModalService) { }
  ...
  caseText: string = "原告向本院提出诉讼请求：1、请求判令被告支付原告货款本金134000元；2、判令被告向原告支付违约金，违约金的计算方式是以合同金额280000元为基数，按照每周千分之一的标准从2016年10月28日起计至实际付清之日止；3、判令被告承担本案诉讼费用。事实与理由：原、被告于2014年签订了1份《销售合同》，在合同期内原告履行了供货义务，一共向被告提供了金额为280000元的货物，并经被告验收合格，但被告仅支付了146000元货款。2017年7月原告派员到被告对账及催还欠款，经被告确认，被告欠原告货款134000元整，同时出具了一份《付款说明函》。截止起诉之日，被告拖欠应付原告货款本金134000元，违约金85680元。被告违反合同的约定，不及时履行付款义务，给原告造成了经济损失。为维护原告合法权益，诉至法院，望判如所请。原告在庭审中明确，原、被告在2014年除签订了上述280000元的合同之外，还签订了一份金额为500000元的合同。";
  ...
  analyze(){
    ...
    this.factors = [];
    this.MmhtService.calcFactors(this.caseText).subscribe((res: any) => {
  ...
  loadCaseRules(){
    ...
    this.MmhtService.getCaseRules(this.factors).subscribe((res: any) => {
      let data = res.data , code = res.code;
      if(code == 200){
    ...
    this.MmhtService.getSameCases(this.factors).subscribe((res: any) => {
      let data = res.data , code = res.code;
      if(code == 200){
  ...
  loadStats(){
    ...
    this.MmhtService.getStats(params).subscribe((res: any) => {
      if(res.code == 200){
        //update geo chart
  ...
  showCaseLaws(c: any, tpl){
    this.modalData.caseObj = c;
    this.MmhtService.getCaseLaws(c.caseId).subscribe((res: any) => {
      if(res.code == 200){
  ...
  showCaseRules(c: any , tpl){
    this.modalData.caseObj = c;
    this.MmhtService.getCaseRules(c.caseKeys).subscribe((res: any) => {
